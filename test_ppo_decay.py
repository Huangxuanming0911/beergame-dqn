...
def run(seed=42, lr_decay=True, ent_decay=True):
    ...
    print(f"lr_decay={lr_decay} ent_decay={ent_decay} seed={seed} eval_mean={eval_mean:.2f}")
    return eval_mean


if __name__ == "__main__":
    run(lr_decay=True, ent_decay=True)
    run(lr_decay=False, ent_decay=True)
    run(lr_decay=True, ent_decay=False)
    run(lr_decay=False, ent_decay=False)
