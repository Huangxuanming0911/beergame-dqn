...
def best_of(seed, n=2):
    best_mean = -float("inf")
    for i in range(n):
        mean, agent = train_and_eval(seed, i)
        if mean > best_mean:
            best_mean = mean
    print(f"seed={seed} best={best_mean:.2f}")
    return best_mean


if __name__ == "__main__":
    means = [best_of(s, n=2) for s in [42, 123, 456]]
    print("best-of-2 mean", np.mean(means), "std", np.std(means))
