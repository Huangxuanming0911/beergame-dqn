...
def train_ppo_best(
    ...
    """训练 PPO，并保存评估奖励最高的检查点。"""
    rng = np.random.default_rng(cfg.get("seed", 42))
    background_policy = str(cfg.get("background_policy", "random"))
    background_target = int(cfg.get("background_base_stock_target", env.config.initial_inventory))
    base_seed = int(cfg.get("seed", 42))
    episodes = int(cfg.get("episodes", 1000))
    rollout_episodes = int(cfg.get("rollout_episodes", 8))
    ...
    scores = []
    agent.total_updates = max(1, episodes // rollout_episodes)
    ...
    for episode in range(1, episodes + 1):
        ...
        if agent.should_update(done=True):
            loss_info = agent.update(state[agent.firm_id])
    ...
    if len(agent.states) > 0:
        agent.update(state[agent.firm_id])
