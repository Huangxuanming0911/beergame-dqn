...
def collect_dqn_value_demos(env, dqn, seed, episodes=50, gamma=0.99):
    """Collect (state, discounted_return) pairs from DQN rollouts."""
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            traj_states.append(state[dqn.firm_id].copy())
            state, rewards, done, _ = env.step(actions_bg)
            traj_rewards.append(float(rewards[dqn.firm_id, 0]))
    ...
    return np.asarray(states, dtype=np.float32), np.asarray(returns, dtype=np.float32)


def pretrain_ppo_value(ppo, states, returns, epochs=50, batch_size=256):
    optimizer = torch.optim.Adam(ppo.net.parameters(), lr=1e-4)
    dataset_size = len(states)
    states_t = torch.FloatTensor(states).to(ppo.device)
    returns_t = torch.FloatTensor(returns).to(ppo.device)
    for _ in range(epochs):
        indices = np.arange(dataset_size)
        np.random.shuffle(indices)
        for start in range(0, dataset_size, batch_size):
            idx = indices[start:start + batch_size]
            _, values = ppo.net.forward(states_t[idx], None)
            loss = nn.functional.mse_loss(values.squeeze(-1), returns_t[idx])
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()


def collect_dqn_actor_demos(env, dqn, seed, episodes=50):
    rng = np.random.default_rng(seed)
    states, actions = [], []
    for ep in range(episodes):
        state = env.reset(seed=seed + ep)
...
def run(seed, pretrain_value=True, pretrain_actor=False):
    ...
    if pretrain_value:
        states, returns = collect_dqn_value_demos(env, dqn, seed, episodes=50)
        pretrain_ppo_value(ppo, states, returns, epochs=50, batch_size=256)
    if pretrain_actor:
        states, actions = collect_dqn_actor_demos(env, dqn, seed, episodes=50)
    ...
    print(f"seed={seed} pretrain_value={pretrain_value} pretrain_actor={pretrain_actor} eval_mean={eval_mean:.2f}")
    return eval_mean


if __name__ == "__main__":
    for s in [42, 123, 456]:
        run(s, pretrain_value=True, pretrain_actor=True)
