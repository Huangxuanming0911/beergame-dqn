...
class PPOPolicy:
    def __init__(self, agent, use_ema=False):
        self.agent = agent
    ...
    def act(self, state, firm_id):
        return self.agent.eval_act(state[firm_id], use_ema=self.use_ema)


def run(use_ema, seed=42):
    ...
    eval_result = evaluate_policy(env, PPOPolicy(agent, use_ema=use_ema), agent.firm_id, episodes=20, seed=seed)
    eval_mean = float(np.mean(eval_result["scores"]))
    print(f"use_ema={use_ema} seed={seed} eval_mean={eval_mean:.2f}")
    return eval_mean
