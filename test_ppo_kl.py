...
def run(seed, target_kl):
    ...
    train_ppo(env, agent, {"seed": seed, "episodes": 1000, "rollout_episodes": 4, "reward_scale": 0.001, "log_every": 200})
    ...
    print(f"target_kl={target_kl} seed={seed} eval_mean={eval_mean:.2f}")
    return eval_mean
...
if __name__ == "__main__":
    for kl in [0.01, 0.02]:
        means = []
        for s in [42, 123, 456]:
            means.append(run(s, kl))
        print(f"target_kl={kl} mean={np.mean(means):.2f} std={np.std(means):.2f}")
